...
export class Line {
    ...
    * @return {Phaser.Line} - This line object.
    */
    fromPoints(start: Point, end: Point): Line {
    ...
    * @return {Phaser.Line} This line object
    */
    rotate(angle: number, asDegrees: boolean = false): Line {
    ...
    * @return {Phaser.Line} This line object
    */
    rotateAround(x: number, y: number, angle: number, asDegrees: boolean = false): Line {
    ...
    */
    intersects(line: Line, asSegment: boolean = true, result?: Point): Point {
    ...
    */
    midPoint(out: Point = new Point()): Point {
    ...
    */
    pointOnLine(x: number, y: number, epsilon: number = 0): boolean {

        return MMaths.fuzzyEqual((x - this.start.x) * (this.end.y - this.start.y), (this.end.x - this.start.x) * (y - this.start.y), epsilon || 0);
    ...
    */
    pointOnSegment(x: number, y: number, epsilon: number = 0): boolean {

        let xMin = Math.min(this.start.x, this.end.x);
        let xMax = Math.max(this.start.x, this.end.x);
        let yMin = Math.min(this.start.y, this.end.y);
        let yMax = Math.max(this.start.y, this.end.y);
    ...
    */
    random(out: Point = new Point()): Point {
    ...
    */
    coordinatesOnLine(stepRate: number = 1, results?: any[]): any[] {
     ...
     */
    clone(output: Line = new Line()): Line {
    ...
    */
    get normalAngle(): number {
        return MMaths.wrap(this.angle - 1.5707963267948966, -Math.PI, Math.PI);
    }
    ...
    *
    * @method static intersectsPoints
    ...
    * @return {Phaser.Point} The intersection segment of the two lines as a Point, or null if there is no intersection.
    */
    static intersectsPoints(a: Point, b: Point, e: Point, f: Point, asSegment: boolean = true, result: Point = new Point()): Point {
    ...
    * @return {Phaser.Point} The intersection segment of the two lines as a Point, or null if there is no intersection.
    */
    static intersects(a: Line, b: Line, asSegment: boolean = true, result?: Point): Point {
    ...
    *
    * @method static intersectsRectangle
    ...
    */
    static intersectsRectangle(line: Line, rect: Rectangle): boolean {
    ...
    *
    * @method static intersectionWithRectangle
    * @param {Phaser.Line} line - The line to check for intersection with.
    ...
    */
    static intersectionWithRectangle(line: Line, rect: Rectangle, result: Point = new Point()): Point {

        let edges = Line.edges;
        let edgeIntersection = Line.edgeIntersection.set(0);
        ...
        for (let edge, i = 0; (edge = edges[i]); i++) {
            if (line.intersects(edge, true, edgeIntersection)) {
                distance = line.start.distance(edgeIntersection);
    ...
    private static edges = [ new Line(), new Line(), new Line(), new Line() ];
    private static edgeIntersection = new Point();

    /**
    * Returns the reflected angle between two lines.
    * This is the outgoing angle based on the angle of Line 1 and the normalAngle of Line 2.
    *
    * @method static reflect

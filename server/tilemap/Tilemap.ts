...
export class Tilemap {
     ...
     */
    public data;
    constructor (public game: Game, public key: string, public tileWidth, public tileHeight, public width, public height) {
        this.data = TilemapParser.parse(this.game, key, tileWidth, tileHeight, width, height);
    ...
    addTilesetImage (tileset, key, tileWidth, tileHeight, tileMargin, tileSpacing, gid) {
        ...
        let idx = this.getTilesetIndex(tileset);
        ...
        {
            let newSet = new Phaser.Tileset(tileset, gid, tileWidth, tileHeight, tileMargin, tileSpacing, {});
            ...
            for (let t = gid; t < gid + newSet.total; t++)
            {
                this.tiles[t] = [ x, y, i ];
    ...
    createFromObjects (name, gid, key, frame, exists, autoCull, group, CustomClass, adjustY, adjustSize) {
        ...
        for (let i = 0; i < this.objects[name].length; i++)
        {
            let found = false;
            let obj = this.objects[name][i];
            ...
            if (found)
            {
                let sprite = new CustomClass(this.game, parseFloat(obj.x, 10), parseFloat(obj.y, 10), key, frame);
    ...
    createFromTiles (tiles, replacements, key, layer, group, properties) {
        ...
        let lw = this.layers[layer].width;
        let lh = this.layers[layer].height;
        ...
        for (let i = 1, len = this._results.length; i < len; i++)
        {
    ...
    createLayer (layer, width, height, group) {
        ...
        let rootLayer = group.add(new Phaser.TilemapLayer(this.game, this, index, width, height));
    ...
    createBlankLayer (name, width, height, tileWidth, tileHeight, group) {
        ...
        let w = layer.widthInPixels;
        let h = layer.heightInPixels;
        ...
        let output = new Phaser.TilemapLayer(this.game, this, this.layers.length - 1, w, h);
        output.name = name;
     ...
     * @param {boolean} [recalculate=true] - Recalculates the tile faces after the update.
     */
    setCollisionByIndex (index, collides, layer, recalculate) {
    ...
    setPreventRecalculate (value) {
        ...
        if (value === false && this.preventingRecalculate === true)
        {
            ...
            for (let i in this.needToRecalculate)
            {
    ...
    calculateFaces (layer) {
        ...
        for (let y = 0, h = this.layers[layer].height; y < h; y++)
        {
            for (let x = 0, w = this.layers[layer].width; x < w; x++)
            {
                let tile = this.layers[layer].data[y][x];
    ...
    searchTileIndex (index, skip, reverse, layer) {
        ...
        if (reverse)
        {
            for (let y = this.layers[layer].height - 1; y >= 0; y--)
            {
                for (let x = this.layers[layer].width - 1; x >= 0; x--)
                {
    ...
    paste (x, y, tileblock, layer) {
        ...
        let diffX = x - tileblock[1].x;
        let diffY = y - tileblock[1].y;

        for (let i = 1; i < tileblock.length; i++)
        {
    ...
    random (x, y, width, height, layer) {
        ...
        for (let t = 1; t < this._results.length; t++)
        {
            if (this._results[t].index)
            {
                let idx = this._results[t].index;
    ...
    dump () {
        ...
        for (let y = 0; y < this.layers[this.currentLayer].height; y++)
        {
            for (let x = 0; x < this.layers[this.currentLayer].width; x++)
            {
                txt += '%c  ';

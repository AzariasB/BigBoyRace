...
class Loader {
    static loadTilemaps() {
        for (let tilemap in Assets.Tilemaps) {
            let tm = Assets.Tilemaps[tilemap];
            if (!this.game.cache.checkTilemapKey(tm.getName())) {
                this.game.load.tilemap(tm.getName(), tm.getJSON(), null, Phaser.Tilemap.TILED_JSON);
            }
        }
    }
    static loadImages() {
    ...
    static orderAudioSourceArrayBasedOnSoundExtensionPreference(soundSourceArray) {
        let orderedSoundSourceArray = [];
        for (let e in this.soundExtensionsPreference) {
            let sourcesWithExtension = soundSourceArray.filter((el) => {
    ...
    static loadAllAssets(game, onComplete, onCompleteContext) {
        this.game = game;
        if (onComplete) {
        ...
        this.loadShaders();
        this.loadMisc();
        if (this.game.load._fileList.length === 0) {
            this.game.load.onLoadComplete.dispatch();
        }
    }
    static waitForSoundDecoding(onComplete, onCompleteContext) {
...
Loader.game = null;
Loader.soundKeys = [];
Loader.soundExtensionsPreference = SOUND_EXTENSIONS_PREFERENCE;

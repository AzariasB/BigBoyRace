...
for (var i in gameAssets) {
    ...
    if (bitmapFontType && imageType) {
        ...
        if(i.indexOf("tilemaps") == 0){
            loaderTypes.tilemap[i] = gameAssets[i];
        }else{
...
if (!Object.keys(loaderTypes.spritesheet).length) {
    ...
    for (var i in loaderTypes.spritesheet) {
        let spritesheetName = i.split('/').pop().split('\.').shift()
        shell.ShellString('\n    export class ' + toPascalCase(spritesheetName) + ' {').toEnd(assetsClassFile);
        shell.ShellString('\n        static getName(): string { return \'' + spritesheetName  +  '\'; }\n').toEnd(assetsClassFile);
...
//Tilemaps
shell.ShellString('export namespace Tilemaps {').toEnd(assetsClassFile);
if(!Object.keys(loaderTypes.tilemap).length){
    shell.ShellString('\n    class IExistSoTypeScriptWillNotComplainAboutAnEmptyNamespace {}').toEnd(assetsClassFile);
}else{
    for(var i in loaderTypes.tilemap){
        var mapName = i.split('/').pop();
        shell.ShellString('\n    export class ' + toPascalCase(mapName) + ' {').toEnd(assetsClassFile);
        shell.ShellString('\n        static getName(): string { return \'' + mapName + '\'; }').toEnd(assetsClassFile);
        shell.ShellString('\n        static getJSON(): string { return require(\'assets/' + i + '.json\'); }').toEnd(assetsClassFile);
        shell.ShellString('\n    }').toEnd(assetsClassFile);
    }
}
